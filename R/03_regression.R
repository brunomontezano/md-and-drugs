...
dados_limpos_para_th <- readr::read_rds("data/clean_data_sem_th.rds")
...
set.seed(123)
receita_th <- recipes::recipe(
    transtorno_humor ~ maconha + cocaina + sexo + cordapele + renda_por_1000 +
        ja_apanhou,
                           data = dados_limpos_para_th) |>
    recipes::step_impute_median(recipes::all_numeric_predictors()) |>
    recipes::step_impute_mode(recipes::all_nominal_predictors()) |>
    recipes::step_dummy(recipes::all_nominal_predictors())
...
fluxo <- workflows::workflow() |>
    workflows::add_model(log_mod) |>
    workflows::add_recipe(receita_th)

fluxo

log_fit <- fluxo |>
    parsnip::fit(data = dados_limpos_para_th)
...
plot_th <- th_plot + ggplot2::labs(title = "Mood disorder",
                                   y = "",
                                  x = "") +
    ggplot2::theme_minimal(12) +
